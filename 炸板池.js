...
!(async () => {
    ...
    * 涨幅	change_rate	199112
    ...
    var config = {
        ...
        url: 'https://data.10jqka.com.cn/dataapi/limit_up/open_limit_pool?page=1&limit=300&field=199112,9002,48,1968584,19,3475914,9003,10&filter=HS,GEM2STAR&order_field=199112&order_type=0&_=1656569328024&date=' + date
    };
    ...
    await api(config).then(async response => {
        ...
        if (body.status_code == 0) {
            if (body.data.info && body.data.info.length > 0) {
                ...
                var message_number = "";
                message_number = `炸板池 \n当前日期：${date}  炸板数量：${body.data.page.total}`

                var message = "";
                for (var i = 0; i < body.data.info.length; i++) {
                    var a = body.data.info[i].change_rate
                    message += `${body.data.info[i].code} ${body.data.info[i].name} 涨幅：${a.toPrecision(3)}%`
